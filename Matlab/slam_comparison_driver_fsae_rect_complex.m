function slam_comparison_driver_fsae_rect_complex()
% SLAM COMPARISON ON ROUNDED RECTANGULAR FSAE TRACK
% EKF SLAM vs FastSLAM vs GraphSLAM
...
%% ========== GENERATE DATA ==========
fprintf('--- Generating Rounded Rectangular Track ---\n');
[time_vec, hx_true, data_log, lm_true] = ...
    generate_fsae_rect_track(DT, SIM_TIME, Q_true, R_true);
...
tic;
x_graph = graph_slam_core(hxDR, hz_graph, 15, 2, 4, deg2rad(8));
t_graph = toc;
...
legend;
title('Rounded Rectangular FSAE Track');
...
function [time_vec, hx_true, data_log, lm] = ...
    generate_fsae_rect_track(dt, T, Q, R)

track_width  = 3.0;
cone_spacing = 2.0;
v_nom = 7.0;

L = 50;        % straight length
Rturn = 12;    % corner radius
...
cx = [cx, s1];
cy = [cy, zeros(size(s1))];
psi = [psi, zeros(size(s1))];

%% ---------- 2) SUZUKA-STYLE S CURVES ----------
Ls = 22;                 % total S length
As = 1.8;                % lateral amplitude (safe)
Ns = round(Ls/(v_nom*dt));
s  = linspace(0, Ls, Ns);

x0 = cx(end);
y0 = cy(end);

% Multiple smooth S oscillations (Suzuka Esses)
yc = As * sin(2*pi*s/Ls) .* sin(pi*s/Ls);
xc = x0 + s;
...
%% ---------- 3) STRAIGHT AFTER S ----------
s2 = dt*v_nom:dt*v_nom:(L - 15);
cx = [cx, cx(end) + s2];
cy = [cy, cy(end)*ones(size(s2))];
psi = [psi, zeros(size(s2))];

%% ---------- 4) FIRST 180 DEG TURN ----------
theta = linspace(0,pi,round(pi*Rturn/(v_nom*dt)));
cx = [cx, cx(end) + Rturn*sin(theta)];
cy = [cy, cy(end) + Rturn*(1-cos(theta))];
psi = [psi, theta];

%% ---------- 5) STRAIGHT BACK ----------
s = 0:dt*v_nom:L;
cx = [cx, cx(end) - s];
cy = [cy, cy(end)*ones(size(s))];
psi = [psi, pi*ones(size(s))];

%% ---------- 6) SECOND 180 DEG TURN ----------
theta = linspace(pi,2*pi,round(pi*Rturn/(v_nom*dt)));
cx = [cx, cx(end) + Rturn*sin(theta)];
cy = [cy, cy(end) + Rturn*(1-cos(theta))];
psi = [psi, theta];
...
for k = 1:N
    ...
    for i=1:size(lm,2)
        ...
        if r<12 && abs(b)<deg2rad(60)
            z_ekf=[z_ekf,[r+randn*sqrt(R(1,1)); b+randn*sqrt(R(2,2))]];
            z_fast=[z_fast,[r;b;i]];
        end
    end
