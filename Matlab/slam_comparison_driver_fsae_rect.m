function slam_comparison_driver_fsae_rect()
...
cx = [cx, s];
cy = [cy, zeros(size(s))];
psi = [psi, zeros(size(s))];

% 2) First 180° turn
theta = linspace(0,pi,round(pi*Rturn/(v_nom*dt)));
cx = [cx, L + Rturn*sin(theta)];
cy = [cy, Rturn*(1-cos(theta))];
psi = [psi, theta];

% 3) Straight back
s = 0:dt*v_nom:L;
cx = [cx, L - s];
cy = [cy, 2*Rturn*ones(size(s))];
psi = [psi, pi*ones(size(s))];

% 4) Second 180° turn
theta = linspace(pi,2*pi,round(pi*Rturn/(v_nom*dt)));
cx = [cx, Rturn*sin(theta)];
cy = [cy, Rturn*(1-cos(theta))];
psi = [psi, theta];
